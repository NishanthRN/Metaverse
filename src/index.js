...
const ambient_light = new THREE.AmbientLight(0xEA526F);
const direction_light = new THREE.DirectionalLight(0x00ff00, 1);
ambient_light.add(direction_light);
scene.add(ambient_light);

const geometry_area = new THREE.BoxGeometry(300, 0.3, 500);
...
polygon.then((result) => {
    result.nft.forEach((object, index) => {
        if (index <= result.supply) {
            ...
            const material_cube = new THREE.MeshPhongMaterial({ color: 0xEA526F });
